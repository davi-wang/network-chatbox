    ...
    ui(new Ui::login)
{
    ...
    //框
    this->setWindowIcon(QIcon(":/img/1.jpg"));//设置框的图标 （路径是 “ 冒号+前缀+路径 ” ）
    ...
    iconNameList<<"1"<<"1"<<"1"<<"1"<<"1"<<"111"<<"222"<<"333"<<"444"<<"555";//这里面写的是图片的名字
    ...
    for(int i=0;i<9;i++)
    {
        ...
        //聊天窗口部分
        btn->setIcon(QPixmap(QString(":/img/%1.jpg").arg(iconNameList[i])));//成员头像
        btn->setIconSize(QPixmap(QString(":/img/%1.jpg").arg(iconNameList[i])).size());//设置图片的大小
